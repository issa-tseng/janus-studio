...
const { sticky } = require('janus-stdlib').varying;
...
class AssemblyView extends DomView.build($(`
  <div class="assembly">
    ...
    <div class="assembly-tools">
      <span class="assembly-hot-reload"/>
      <button class="assembly-save">&#x2935;</button>
    </div>
    <div class="assembly-statements"/>
    <div class="assembly-main">
      <div class="assembly-pin"/>
      <div class="assembly-stacks">
        <div class="stacks"/>
        <button class="new-stack">+</button>
      </div>
    </div>
    <div class="console">
  ...
  find('.assembly').classed('has-pin', from('pin').map(exists)),

  find('.assembly-name').render(from.attribute('name')),

  find('.assembly-hot-reload').render(from.attribute('ui-hot-reload'))
    .criteria({ style: 'button' }).options({ stringify: _ => '\u21bb' }),
  find('.assembly-save')
    .classed('saved', from('ui-saved').pipe(sticky({ true: 2000 })))
    .on('click', (_, assembly) => { assembly.save(); }),

  find('.assembly-statements').render(from('statements')),

  find('.assembly-pin').render(from('pin')),
  find('.assembly-stacks .stacks').render(from('stacks')),
  find('.new-stack').on('click', (_, assembly) => { assembly.get_('stacks').add(new Stack()); }),
  ...
  _initialize() {
    if (!this.subject.get_('freshness')) this.subject.run();
  }
  _wireEvents() {
