...
const { Block, Stacks, Stack } = require('./block');
...
class Statement extends Model.build(
  ...
  serialize() {
    // enforce id reification. TODO: better mechanism.
    this.get_('id');
...
class Assembly extends Model.build(
  ...
  run() {
    // TODO: do we even want to auto-rerun on context change?
    const context = this.get_('project').get_('context');

    // start by resetting our requires.
    for (const reset of context.get_('reset')) {
      const resolved = resolve(reset);
      for (const key of Object.keys(require.cache))
        if (key.startsWith(resolved)) delete require.cache[key];
    }

    // then compile and execute our root environment code.
    const init = context.get_('init-bindings').successOrElse({});
    let bindings = compile(init, context.get_('env')).flatMap(f => f());
    bindings = bindings.flatMap(scope => success(Object.assign({}, janus, init, scope)));
  ...
  pin(block) {
    this.unpin();
    this.set('pin', block);
    for (const stack of this.get_('stacks')) stack.get_('blocks').remove(block);
  }
  unpin() {
    const block = this.get_('pin');
    if (!block) return;

    const stacks = this.get_('stacks');
    if (stacks.length_ === 0) stacks.add(new Stack());
    stacks.get_(0).get_('blocks').add(block);

    this.unset('pin');
  }
  togglePin(block) {
    if (this.get_('pin') === block) this.unpin();
    else this.pin(block);
  }
