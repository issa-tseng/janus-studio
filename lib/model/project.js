...
const { compile, fail } = require('../exec/eval');
const { Listing, content } = require('../util/fs');
const { ifexists } = require('../util/util');


const Context = Model.build(
  bind('init-bindings', from('init').map(code =>
    tap(code) ? compile({}, code).flatMap(f => f()) : fail()
  ))
);
...
class Project extends Model.build(
  ...
  bind('context', from('config').get('context').map(x => new Context(x))),
