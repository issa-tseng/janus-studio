...
const { Statement } = require('../model/project');
...
// viewmodel companion to Reference
class Entanglement extends Model.build(
) {
}
...
class Polymer extends Model.build(
  ...
  getAtom(id) {
    for (const atom of this.get_('atoms'))
      if (atom.get_('statement').get_('id') === id) return atom;
  }
