...
const inspect = (x, y) => {
  const result = ((x != null) && (x.isInspector === true)) ? x : jInspect.inspect(x);
  if (result && (typeof result.set === 'function')) result.set('_parent', y);
  return result;
}
...
const trace = (inspector) => {
  const { Block } = require('../model/machine'); // TODO: blah.
  const links = [];
  let ptr = inspector;
  do {
    if (ptr.isBlock) return new Block({ to: ptr.get_('to'), links: new List(links) });
    if (ptr.isStatement) return new Block({ to: ptr, links: new List(links) });

    const reference = ptr.get_('_reference');
    if (reference) links.push(reference);
  } while (ptr = ptr.get_('_parent'));
};
  ...
  app.on('inspected', (view) => {
    ...
    dom.find('.panel-handle').on('mousedown', (event) => {
      ...
      if (block_.length) {
        ...
        drag(event, view, false, trace(view.subject));
      }
    });
  });
