...
const trace = (view) => {
  const { Block } = require('../model/block'); // TODO: blah.
  const links = [];
  let ptr = view;
  do {
...
const handle = `<div class="panel-handle"/>`;
const decorateInspectors = (app) => {
  ...
  app.on('inspected', (view) => {
    ...
    // HACK: propagate handle: false option
    if (view.options.trigger != null) {
      const parent = view.options.trigger.closest('.janus-inspect-panel');
      if (parent.length) view.options.handle = parent.view().options.handle;
    }

    // inject handle
    const dom = view.artifact();
    const wantsHandle = (view.options.handle !== false);
    dom.find('.janus-inspect-pin').replaceWith(wantsHandle ? handle : '');
    ...
    dom.find('.panel-handle').on('mousedown', (event) => {
      ...
      if (block_.length) {
        ...
        if (block.get_('type') === 'reference') {
          const clone = !!dom.closest('.statement').length;
