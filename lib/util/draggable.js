...
class Drag extends Model.build(
  ...
  constructor(view, clone, drop, dom, x, y) {
    super({ view, clone, drop, dom, startx: x, starty: y, x, y });
  }
  ...
  seek(stacks) { return (event) => {
    ...
    for (const stack_ of stacks.children()) {
      ...
      if (!(self.parents().is(stack)) || self.next().length) {
  ...
  commit() {
    // get basic information; abort if nothing valid is targeted.
    ...
    // remove from possible old parents.
    const parent = view.closest_(List);
    if (parent) parent.subject.remove(source);

    const { Assembly } = require('../model/assembly');
    const assemblyView = view.closest_(Assembly);
    if (assemblyView) {
      const assembly = assemblyView.subject;
      if (assembly.get_('pin') === view.subject) assembly.unset('pin');
    }
    ...
    const drop =
      this.get_('drop') ||
      ((this.get_('clone') === true) ? source.shadow() : source);
...
const drag = (event, view, clone, drop) => {
  event.preventDefault();
  const dom = view.artifact();
  dom.addClass('dragging');
  const op = new Drag(view, clone, drop, dom, event.pageX, event.pageY);
  ...
  const stacks = $('.assembly-stacks .stacks > .janus-list'); // TODO: still not right.
