...
const delayedHover = (dom, selector, cb) => {
  dom.on('mouseenter', selector, event => {
    const trigger = $(event.currentTarget);
    const timer = setTimeout(cb.bind(null, trigger), 300);
    trigger.one('mouseleave', _ => { clearTimeout(timer); });
  });
};

module.exports = { positionFlyout, delayedHover };
